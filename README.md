...
- The exact one-sided sign test is `p = 0.125` within each three-seed cohort and is used only as a directional diagnostic. Because the pooled direction was not preregistered, the six-of-six pooled result is reported with a two-sided sign-test value of `p = 0.0313` and treated as supporting seed replication rather than an independent confirmation.
